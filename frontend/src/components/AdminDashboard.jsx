                                        ...
                                        <tr key={user._id} className="hover:bg-slate-50/50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-xs font-medium">{user.phone}</span>
                                                    <span className="text-xs text-slate-400 lowercase">{user.email}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                        ...
                        <div className="p-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-6">
                                    ...
                                    <DetailItem icon={<Briefcase className="text-purple-500" />} label="Occupation" value={selectedUser.occupation} />
                                    <DetailItem icon={<Calendar className="text-purple-500" />} label="Date of Birth" value={selectedUser.dob} />
                                    <DetailItem icon={<Phone className="text-purple-500" />} label="Phone" value={selectedUser.phone} />
                                    <DetailItem icon={<Mail className="text-purple-500" />} label="Email" value={selectedUser.email} />
                                    <DetailItem icon={<CreditCard className="text-purple-500" />} label="Aadhaar" value={selectedUser.aadhaar} />
                                    <DetailItem icon={<FileText className="text-purple-500" />} label="PAN" value={selectedUser.pan} />
                                    <DetailItem icon={<UserIcon className="text-purple-500" />} label="Father's Name" value={selectedUser.fatherName} />
                                    <DetailItem icon={<UserIcon className="text-purple-500" />} label="Mother's Name" value={selectedUser.motherName} />
                                </div>

                                <div className="space-y-6 flex flex-col">
                                    <div className="flex-1">
                                        <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                                            <Download size={14} className="text-purple-500" /> Payment Screenshot
                                        </h3>
                                        <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-xl bg-slate-50 group relative">
                                            <img
                                                src={selectedUser.paymentScreenshot}
                                                alt="Payment"
                                                className="w-full h-auto object-contain min-h-[500px] max-h-[750px] transition-transform duration-500 group-hover:scale-105"
                                            />
                                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors pointer-events-none"></div>
                                        </div>
                                    </div>
                                    <a
                                        href={selectedUser.paymentScreenshot}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center justify-center gap-3 w-full py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg hover:shadow-slate-200 active:scale-[0.98]"
                                    >
                                        <Download size={20} /> Download Full Image
                                    </a>
                                </div>
                            </div>
                        </div>
